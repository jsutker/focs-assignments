...
1. ∃x. ∀y. loves(x, y): No, no letter is x 4 times
2. ∃y. ∀x. loves(x, y): Yes, there are x's a, b, c, and d, for which y is the same (in this case, c)
3. ∀x. ∃y. loves(x, y): Yes, each letter appears as an x with a y
4. ∀y. ∃x. loves(x, y): Yes, each letter appears as a y with an x
5. ∃x. ∃y. loves(x, y): Yes, there is an x that has a y
6. ∃x. ∀y. ¬loves(x, y): No, see #3
7. ∃x. ¬∀y. loves(x, y): Yes, there is an x that doesn't have all possible y's (in this case, all of them)
